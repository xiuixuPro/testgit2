        ...
        <div class="box col-md-12">
            <div class="box-inner">
                <div class="box-header well" data-original-title="">
                    <h2><i class="glyphicon glyphicon-user"></i>编辑用户表</h2>
				...
				-->
                   <form name="myform" method="post" action="<?php echo U('User/editact');?>" enctype="multipart/form-data" />
			<input type="hidden" value="<?php echo $a['userid']?>" name="userid">
                   
					 <div class="form-group">
                        <label for="exampleInput7">用户名称</label><br>
                      
					  <input required='required' type="text" value="<?php echo $a['username'];?>" class="form-control" id="exampleInput1" name="username" ><span id="dsitename"></span>
                  
                 
                 
				   </div>
				   <div class="form-group">
                        <label for="exampleInput5">权限</label><br>
                     
						 <select name="groupid">
									<?php if(is_array($b)): $i = 0; $__LIST__ = $b;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$p): $mod = ($i % 2 );++$i;?><option
											<?php if($a['roleid']==$p["roleid"]) echo "selected";?>	

										value="<?php echo $p['roleid']?>"> <?php echo $p['rolename']?> </option><?php endforeach; endif; else: echo "" ;endif; ?>
						</select>
				   </div>
